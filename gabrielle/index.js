...
app.post('/api/inscricao', async (req, res) => {
    ...
    try {
        ...
        if (inscricaoExistente) {
            return res.status(400).json({ sucesso: false, mensagem: 'Usuário já inscrito neste curso.' });
        }
        ...
        res.status(201).json({ sucesso: true, mensagem: 'Inscrição realizada com sucesso!' });
    } catch (error) {
...
// Endpoint para buscar as inscrições do usuário
app.get('/api/inscricoes/:id', async (req, res) => {
    const { id } = req.params;
    
    try {
        // Usando o Sequelize para buscar as inscrições do usuário
        ...
        // Retornando as inscrições do usuário
        return res.json(inscricoes);  
    } catch (err) {
        // Se ocorrer algum erro durante a consulta
...
// Rota para obter a quantidade de inscrições de um usuário
app.get('/api/inscricoes/quantidade', async (req, res) => {
    ...
    try {
        // Conta o número de inscrições para o usuário
        const quantidade = await Inscricao.count({
            where: { id: usuarioId }
        });
