...
function App() {
    ...
    useEffect(() => {
        // Função para obter a quantidade de inscrições
        const getQuantidadeInscricoes = async () => {
            try {
                const usuarioId = usuarioLogado ? usuarioLogado.id : null;  // Verifica se o usuário está logado
        
                const response = await fetch(`/api/inscricoes/quantidade?usuarioId=${usuarioId}`);
        
                // Verifica se a resposta não é uma página de erro HTML
                if (!response.ok) {
                ...
                // Atualiza o estado com a quantidade de inscrições
                setInscricaoLateral(data.quantidade);
            } catch (error) {
        ...
        getQuantidadeInscricoes();  // Chama a função quando o componente é montado ou quando o usuário logado muda

    }, [usuarioLogado]);  // Refaz a requisição quando o usuário logado mudar
